...
<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm" role="dialog" aria-modal="true" tabindex="-1" aria-labelledby="restock-title">
	<div class="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-3">
		...
		<span class="block text-sm font-semibold">Produk</span>
		<select aria-label="Produk" bind:value={productId} class="w-full px-3 py-2 border rounded-lg"><option value={0}>— Pilih —</option>{#each products as p (p.id)}<option value={p.id}>{p.name}</option>{/each}</select>
		<span class="block text-sm font-semibold">Jumlah</span>
		<input aria-label="Jumlah" type="number" bind:value={qty} min="1" class="w-full px-3 py-2 border rounded-lg" />
		<span class="block text-sm font-semibold">Alasan</span>
		<input aria-label="Alasan" type="text" bind:value={reason} placeholder="Contoh: PO supplier" class="w-full px-3 py-2 border rounded-lg" />
		<span class="block text-sm font-semibold">No. PO (optional)</span>
		<input aria-label="No. PO (optional)" type="text" bind:value={referenceId} class="w-full px-3 py-2 border rounded-lg" />
